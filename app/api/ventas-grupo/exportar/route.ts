...
function tipoVisible(o: any) {
  return o.tipo === 'PORTA' && o.operaciones_porta?.es_linea_nueva ? 'LN' : o.tipo
}

function nombreCliente(c: any) {
  if (!c) return '-'
  return [String(c.apellido ?? '').trim(), String(c.nombre ?? '').trim()].filter(Boolean).join(', ') || '-'
}

function diaAR(v: any) {
  if (!v) return ''
...
export async function POST(request: Request) {
  try {
    ...
    const autorizado =
      profile?.activo === true &&
      ['VENDEDOR', 'TERRENO'].includes(String(profile.rol)) &&
      !!sigla &&
      sigla !== 'L1'

    if (!autorizado || !sigla) {
      return NextResponse.json({ error: 'No tiene permisos para exportar Ventas del Grupo.' }, { status: 403 })
    }
    ...
    const productos: string[] = Array.isArray(body.productos)
      ...
      : []
    const baseFecha = String(body.base_fecha ?? '')
    const formato = String(body.formato ?? '').toLowerCase()
    const camposExportacion = String(body.campos_exportacion ?? 'vista').toLowerCase()

    if (!/^\d{4}-\d{2}-\d{2}$/.test(desde) || !/^\d{4}-\d{2}-\d{2}$/.test(hasta) || desde > hasta) {
      return NextResponse.json({ error: 'Rango de fechas inválido.' }, { status: 400 })
    }
    if (productos.length === 0 || productos.some((p) => !PRODUCTOS_VALIDOS.has(p))) {
      return NextResponse.json({ error: 'Selección de productos inválida.' }, { status: 400 })
    }
    if (!BASES_VALIDAS.has(baseFecha)) return NextResponse.json({ error: 'Base de fecha inválida.' }, { status: 400 })
    if (productos.includes('BAF') && baseFecha !== 'fecha_ingreso') {
      return NextResponse.json({ error: 'Con BAF seleccionado solo puede usarse Fecha Ingreso.' }, { status: 400 })
    }
    if (productos.includes('LN') && baseFecha === 'fecha_porta') {
      return NextResponse.json({ error: 'Fecha Porta solo está disponible para PORTA sin Línea Nueva.' }, { status: 400 })
    }
    ...
    const { data: vista, error: errorVista } = await admin
    ...
    const configuracion = (vista ?? []).map((c: any) => ({
      campo: String(c.campo), etiqueta: String(c.etiqueta || c.campo), visible: Boolean(c.visible),
      orden: Number(c.orden) || 0, ancho: Math.min(600, Math.max(60, Number(c.ancho) || 140)),
    }))
    const columnasVisibles = configuracion.filter((c: any) => c.visible)
    if (!columnasVisibles.length) return NextResponse.json({ error: 'La vista no tiene columnas visibles.' }, { status: 400 })
    const columnas = camposExportacion === 'todos' ? configuracion : columnasVisibles
    if (!columnas.length) return NextResponse.json({ error: 'No hay campos disponibles para exportar.' }, { status: 400 })
    ...
    const idsEO = Array.from(new Set((rGPorta.data ?? []).map((g: any) => g.estado_bboo_id).filter(Boolean)))
    ...
    const completas = operaciones.map((o: any) => {
      const gB: any = gb.get(o.id_operacion), gP: any = gp.get(o.id_operacion)
      return {
        ...o,
        ...
        gestion_porta: gP ? {
          ...gP,
          estado_vendedor_nombre: (ep.get(gP.estado_porta_id) as any)?.nombre || null,
          estado_bboo_nombre: (eo.get(gP.estado_bboo_id) as any)?.nombre || null,
        } : null,
      }
    })

    const responsable = (o: any) => {
      const id = o.tipo === 'BAF' ? o.gestion_baf?.responsable_id : o.gestion_porta?.responsable_id
      const n = nombrePerfil(id)
      return n === '-' ? 'Sin responsable' : n
    }

    const valor = (o: any, c: string) => {
      const p = o.operaciones_porta, g = o.gestion_porta, b = o.gestion_baf
      switch (c) {
        case 'fecha_ingreso': return o.fecha_hora || null
        case 'tipo': return tipoVisible(o)
        case 'vendedor': return o.vendedor || '-'
        case 'responsable': return responsable(o)
        ...
        case 'plan_acordado': return o.tipo === 'PORTA' ? p?.gigas_acordados || '-' : '-'
        case 'plan_cargado': return o.tipo === 'PORTA' ? g?.plan_cargado || '-' : '-'
        case 'estado_vendedor': return o.tipo === 'PORTA' ? g?.estado_vendedor_nombre || 'Sin gestión' : '-'
        case 'estado_bboo': return o.tipo === 'PORTA' ? g?.estado_bboo_nombre || 'Sin gestión' : '-'
        case 'estado_baf': return o.tipo === 'BAF' ? b?.estado_nombre || 'Sin gestión' : '-'
        case 'bboo': return o.tipo === 'PORTA' ? nombrePerfil(g?.bboo_id) : '-'
        case 'fecha_carga_stl': return o.tipo === 'PORTA' ? g?.fecha_carga_stl || null : null
        case 'fecha_porta': return o.tipo === 'PORTA' && !p?.es_linea_nueva ? g?.fecha_porta || null : null
        case 'medio_despacho_chip': return o.tipo === 'PORTA' && g?.medio_despacho_chip_id ? (medios.get(g.medio_despacho_chip_id) as any)?.nombre || '-' : '-'
        case 'numero_seguimiento': return o.tipo === 'PORTA' ? g?.numero_seguimiento || '-' : '-'
        case 'pin': return o.tipo === 'PORTA' ? g?.pin_lnva_nro || '-' : '-'
        case 'sim_operativo': return o.tipo === 'PORTA' ? g?.sim || '-' : '-'
    ...
    const filtradas = completas.filter((o: any) => {
      const t = tipoVisible(o)
      if (!productos.includes(t)) return false
      const fv = baseFecha === 'fecha_ingreso' ? o.fecha_hora : baseFecha === 'fecha_carga_stl' ? o.gestion_porta?.fecha_carga_stl : o.gestion_porta?.fecha_porta
      const d = diaAR(fv)
      return d && d >= desde && d <= hasta
    })

    const criterios = columnasVisibles.slice(0, 2).map((c: any) => c.campo)
    const vacio = (v: any) => v == null || String(v).trim() === '' || String(v).trim() === '-'
    const comparar = (a: any, b: any, c: string) => {
      const va = valor(a, c), vb = valor(b, c), ea = vacio(va), ebv = vacio(vb)
      if (ea && ebv) return 0
      if (ea) return 1
      if (ebv) return -1
      if (CAMPOS_FECHA.has(c)) {
        const ta = new Date(String(va)).getTime(), tb = new Date(String(vb)).getTime()
        if (!Number.isNaN(ta) && !Number.isNaN(tb)) return tb - ta
      }
      return String(va).localeCompare(String(vb), 'es', { sensitivity: 'base', numeric: true })
    }
    filtradas.sort((a: any, b: any) => {
      for (const c of criterios) { const r = comparar(a, b, c); if (r) return r }
      return new Date(b.fecha_hora || 0).getTime() - new Date(a.fecha_hora || 0).getTime()
    })

    const nombreBase = `ventas_grupo_${sigla}_${desde}_${hasta}`
    if (formato === 'csv') {
      const esc = (v: any) => { const s = String(v ?? ''); return /[",\n\r;]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s }
      const filas = [
        columnas.map((c: any) => esc(c.etiqueta)).join(';'),
        ...filtradas.map((o: any) => columnas.map((c: any) => {
          const v = valor(o, c.campo)
          return esc(CAMPOS_FECHA.has(c.campo) ? diaAR(v) : v ?? '')
        }).join(';')),
      ]
      return new Response('\uFEFF' + filas.join('\r\n'), {
        headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': `attachment; filename="${nombreBase}.csv"` },
      })
    }

    const wb = new ExcelJS.Workbook()
    wb.creator = 'Plataforma Lucom'
    const ws = wb.addWorksheet('Ventas del Grupo', { views: [{ state: 'frozen', ySplit: 1 }] })
    ws.columns = columnas.map((c: any) => ({ header: c.etiqueta, key: c.campo, width: Math.max(10, Math.min(80, Math.round(c.ancho / 7))) }))
    for (const o of filtradas) {
      const fila: any = {}
      for (const c of columnas) {
        const v = valor(o, c.campo)
        fila[c.campo] = CAMPOS_FECHA.has(c.campo) ? fechaExcel(v) : v === '-' ? '' : v
      }
      ws.addRow(fila)
    }
    ...
    for (const c of columnas) {
      if (CAMPOS_FECHA.has(c.campo)) ws.getColumn(c.campo).numFmt = c.campo === 'fecha_ingreso' ? 'dd/mm/yyyy hh:mm' : 'dd/mm/yyyy'
    }
    const buffer = await wb.xlsx.writeBuffer()
    return new Response(buffer as any, {
      headers: { 'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'Content-Disposition': `attachment; filename="${nombreBase}.xlsx"` },
    })
  } catch (e: any) {
    console.error('exportar ventas grupo', e)
    return NextResponse.json({ error: e?.message || 'No se pudo generar la exportación.' }, { status: 500 })
  }
}
