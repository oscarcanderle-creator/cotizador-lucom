...
import BandejasVentasGrupo from '../../components/BandejasVentasGrupo'
import ExportarVentasGrupo from '../../components/ExportarVentasGrupo'
...
export default async function VentasGrupoPage({
  searchParams,
}: {
  ...
  const extraerSigla = (valor: unknown) => {
    const primerToken = String(valor ?? '').trim().split(/\s+/)[0]?.toUpperCase() || ''
    return /^[A-Z0-9]{2}$/.test(primerToken) ? primerToken : null
  }

  const sigla = extraerSigla(profile.vendedor)
  const esRolGrupo = profile.rol === 'VENDEDOR' || profile.rol === 'TERRENO'

  // Ventas del Grupo es un circuito independiente y exclusivamente de lectura.
  // L1 no utiliza esta sección: conserva el circuito actual de Gestión de Ventas.
  if (!esRolGrupo || !sigla || sigla === 'L1') redirect('/ventas')

  const esBboo = false
  const admin = createAdminClient()
  const rolVista = 'VENTAS_GRUPO'
  ...
  const operaciones = (operacionesBase ?? []).filter((o: any) => extraerSigla(o.vendedor) === sigla)
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Vista de solo lectura de las ventas correspondientes al grupo {sigla}.
          </p>
        </div>

        <ExportarVentasGrupo puedeExportar={true} />
              ...
              {ventas.map((operacion: any) => (
                ...
                >
                  ...
                  <td className="px-2 py-3 text-right">
                    <a
                      href={`/ventas-grupo/${encodeURIComponent(
                        operacion.id_operacion
                      )}`}
